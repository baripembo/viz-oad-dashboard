...
function setKeyFigures() {
	...
	//vaccine rollout
		...
		createKeyFigure('.figures', 'Number of Countries', '', totalCountries);
		createKeyFigure('.figures', 'COVAX Allocations Round 4 – 6 (Number of Doses)', '', data['#capacity+doses+forecast+covax']==undefined ? 'NA' : shortenNumFormat(data['#capacity+doses+forecast+covax']));
...
function setGlobalLegend(scale) {
  ...
  if (d3.select('.map-legend.global .scale').empty()) {
    ...
    //pin footnote
    createFootnote('.map-legend.global', '#affected+inneed+pct', 'The Total Number of People in Need figure corresponds to 30 (instead of the current 27) HRPs and 7 Regional Appeals. Population percentages greater than 100% include refugees, migrants, and/or asylum seekers.');
    //vacc footnote
